...
array_index(AV* av, PyObject *v)
{
	...
	if (svp) {
	    ...
	    x = sv2pyo(*svp);
	    PERL_UNLOCK;
	    int cmp = PyObject_RichCompareBool(x, v, Py_EQ);
	    /* https://stackoverflow.com/questions/29175295/python-compare-objects-in-c-api
	       1 is returned for true, 0 for false, and -1 for an error.*/
	    Py_DECREF(x);
	    if (cmp == 1) {
		ENTER_PERL;
		return i;
...
pysvrv_count(PySVRV *self, PyObject *args)
{
	...
	if (svp) {
	    ...
	    x = sv2pyo(*svp);
	    PERL_UNLOCK;
	    cmp = PyObject_RichCompareBool(x, v, Py_EQ); /*see above*/
	    Py_DECREF(x);
	    if (cmp == 1)
		count++;
	    if (cmp == -1 && PyErr_Occurred()) {
...
pysvrv_getattr(PySVRV *self, char *name)
{
	...
	char *tmp;
	ENTER_PERL;
	tmp = (char*)sv_reftype(SvRV(self->rv), 0);
	ENTER_PYTHON;
	...
	printf("!!!val = Py_FindMethod(list_methods, (PyObject *)self, name);\n");//AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
    }
    else if (SvTYPE(SvRV(self->rv)) == SVt_PVHV) {
	printf("val = Py_FindMethod(mapp_methods, (PyObject *)self, name);\n");//AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
    }
    else {
	PyErr_SetString(PyExc_AttributeError, name);
	val = NULL;
...
PyTypeObject SVRVtype = {
    ...
    //_PyObject_EXTRA_INIT 1, &PyType_Type,
