...
extern void boot_Python__Object(pTHXo,CV*);

#ifdef WIN32_NOT
SV* (*pnewPerlPyObject_inc)(PyObject *py);
#endif

PerlInterpreter *glob_main_perl = 0;
...
{
    PerlInterpreter *p;
    //char *embedding[] = { "", "-Mblib", "-mPython::Object", "-e", "0", NULL };
    ...
    {
	HMODULE m = GetModuleHandle("Object.xs.dll");
     ...
     PyObject *args;
{
    ...
    SV* res_sv = perl_eval_pv(code, FALSE);
    int errsv = SvTRUE(ERRSV);
...
PyInit_perl (void) {
    ...
    perl_parse(main_perl, xs_init, 3, embedding, NULL);
    //printf("PyInit_perl 4\n");
    perl_run(main_perl);
    //printf("PyInit_perl 5\n");
#endif
...
#endif
{
    ...
    PyObject *m = PyModule_Create(&moduledef);
#else
    PyObject *m = Py_InitModule("perl", PerlMethods);
#endif
...
newPerlPyObject_noinc(PyObject *pyo)
{
    ...
    PerlInterpreter *my_perl = glob_main_perl;//////////HAAACK INCORRECT for
	    //multiple perl instances; //but we dont want much instances anyway
    SV* rv = newSV(0);
...
SV*
vk_newPerlPyObject_inc(PyObject *pyo)
{
    SV* sv;
    dCTX;
    ASSERT_LOCK_BOTH;
